    ...
    switch (bitsPerSample) {
        case 8:
            for (float sample : data) {
                uint8_t val = static_cast<uint8_t>((sample + 1.0) * 127.5);
                byteData.push_back(val);
            }
            break;
        case 16:
            for (float sample : data) {
...
std::vector<float> ProcessRecording(const std::vector<uint8_t>& audioData, int sampleRate, int channels, int sampleSize, bool saveRecording) {
    ...
    // Simulate reading the WAV file back (assuming conversion was needed)
...
// int main() {
//     // Example usage
//     std::vector<uint8_t> fakeAudioData(1024, 128); // Fake audio data
//     std::vector<float> samples = ProcessRecording(fakeAudioData, 44100, 2, 16, true);

//     std::cout << "Generated Unique ID: " << GenerateUniqueID() << std::endl;
//     std::cout << "Generated Song Key: " << GenerateSongKey("SongTitle", "ArtistName") << std::endl;
